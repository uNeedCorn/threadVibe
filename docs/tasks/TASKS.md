...
### BE-02：Threads API Views 延遲問題處理 [普通]

**目標：** 處理 Threads API 新貼文 views = 0 但有互動數的問題

**問題描述：**
- Threads API 對新貼文（約前 15~30 分鐘）會回傳 `views: 0`
- 但同時會正確回傳互動數（likes, replies, reposts, quotes）
- 這是 Threads API 的已知限制，非程式 bug
- 實測：貼文發布 28-35 分鐘後，views 開始正常回傳（2026-01-12 測試）

**目前處理：**
- UI 已添加「計算中」提示（當 views = 0 且有互動數時顯示）
- 四象限圖過濾掉 views = 0 的貼文

**待研究方向：**
- [ ] 考慮使用估算公式（如：預估 views = 互動數 × 平均互動率倒數）
- [ ] 或等待足夠資料點後再計算 virality score
- [ ] 或在 views = 0 時使用純互動數排名

**參考資料：**
- 資料庫記錄：`workspace_threads_post_metrics_15m` 表 views = 0、likes > 0
- API 呼叫時間：2026-01-12 04:15 UTC
...
**目標：** 實作 Threads 貼文撰寫與即時發布功能

**需求：**
- 貼文撰寫介面：
  - 文字編輯器（支援字數統計、500 字上限提示）
  - 媒體上傳（圖片/影片，支援多張）
  - 預覽模式（模擬 Threads 樣式）
  - 帳號選擇（多帳號時選擇發布來源）
- 發布流程：
...
**技術考量：**
- 使用 Threads API Publishing 端點
- 媒體上傳流程（Container → Publish）
- 錯誤處理與重試機制

**相關 API：** Threads Publishing API
...
- 排程管理：
  ...
  - 排程狀態（待發布/發布中/已發布/失敗）
- 發布執行：
  - 使用 pg_cron 或 Edge Function 定時觸發
  - 發布失敗通知

**資料表需求：**
- `scheduled_posts` 表（儲存排程貼文內容與時間）

...
除了 Claude Code 的 MCP 通知外，系統也透過 Edge Functions 發送重要事件通知。

### 自動通知事件

| 事件 | 說明 | 觸發點 |
|------|------|--------|
| `new_user` | 新用戶註冊 | `auth/callback` |
| `threads_connected` | Threads 帳號連結 | `threads-oauth-callback` |
...
管理員可在 API 測試頁面（`/admin/api-test`）測試系統通知功能：

1. 前往 `/admin/api-test`
2. 找到「Telegram 通知測試」區塊
3. 選擇通知類型（測試訊息 / 新用戶註冊 / Threads 連結）
4. 點擊「發送測試」
5. 檢查 Telegram 是否收到通知
...
| 檔案 | 說明 |
|------|------|
| `supabase/functions/_shared/notification.ts` | 通知核心模組 |
| `supabase/functions/user-events/index.ts` | 使用者事件處理 |
| `frontend/app/api/admin/telegram-test/route.ts` | 測試 API |

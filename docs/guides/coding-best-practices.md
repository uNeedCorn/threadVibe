...
### 錯誤訊息安全原則（Edge Functions）

**重要：後端錯誤訊息不得暴露內部細節給前端使用者。**
...
// ✅ 正確：通用訊息
console.error('Account query error:', { accountId, error });
return errorResponse(req, '找不到此帳號', 404);
```

| 錯誤類型 | 使用者可見訊息 | console.error 記錄 |
|----------|----------------|-------------------|
| API 錯誤 | `服務暫時無法使用` | 完整 status + response |
| 設定錯誤 | `系統設定錯誤，請聯繫管理員` | 缺少的環境變數名稱 |
| 資料庫錯誤 | `操作失敗，請稍後再試` | 完整錯誤物件 |
| 找不到資源 | `找不到此資源` | 資源 ID + 錯誤詳情 |
| 權限錯誤 | `無權限執行此操作` | 使用者 ID + 嘗試的操作 |
